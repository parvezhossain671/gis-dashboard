...
async function loadBooks(){
  ...
  try{
    ...
    book.innerHTML="";
    book.add(new Option("Select Book No",""));
    ...
    book.innerHTML="";
    book.add(new Option("Unable to load Book No",""));
    book.disabled=false;
...
book.onchange=async()=>{
  account.innerHTML="";
  account.add(new Option(book.value?"Loading Account No…":"Select Book No first",""));
  account.disabled=true;
  hideResult();
  if(!book.value)return;
  try{
    const d=await pagedSelect(`locations?select=%22Account%20No%22&%22Book%20No%22=eq.${encodeURIComponent(book.value)}`);
    const v=[...new Set(d.map(x=>String(x["Account No"]??"").trim()).filter(Boolean))].sort((a,b)=>a.localeCompare(b,undefined,{numeric:true}));
    account.innerHTML="";
    account.add(new Option("Select Account No",""));
    v.forEach(x=>account.add(new Option(x,x)));
    account.disabled=false;
    setStatus(status,`${v.length} Account No available.`,v.length?"success":"");
  }catch(e){
    console.error("load accounts",e);
    account.innerHTML="";
    account.add(new Option("Unable to load Account No",""));
    account.disabled=true;
